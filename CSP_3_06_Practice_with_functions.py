...
def multiplyDifferences(d, e, f, g):
    return d*(d-e)*(d-f)*(d-g)

#Given the 3 sides of a triangle return the area.
#use herons formula
#Use the functions above.
def herons(h, i, j):
    sp = semiPerimeter(h, i, j)
    md = multiplyDifferences(sp, h, i, j)
    return root(md)
...
def quadratic(a, b, c):
    mc = mainCalc(a, b, c)
    rt = root(abs(mc))
    n1, n2 = plusMinus(b, rt)
    return n1/denominator(a), n2/denominator(a)
